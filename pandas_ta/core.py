    ...
    def _append(self,
        ...
        if isinstance(result, DataFrame):
            ...
            if "col_names" in kwargs and isinstance(kwargs["col_names"], tuple):
                    ...
                    for col, ind_name in zip(result.columns, kwargs["col_names"]):
                        try:
                            ...
                            try:
                                ...
                                converted = to_numeric(result.loc[:, col], errors='coerce')
                                converted = converted.replace([float('inf'), float('-inf')], float('nan'))
                                converted = converted.fillna(0.0)
                                df[ind_name] = converted
                            except:
                ...
                for i, column in enumerate(result.columns):
                    try:
                        ...
                        try:
                            from pandas import to_numeric
                            converted = to_numeric(result.iloc[:, i], errors='coerce')
                            converted = converted.replace([float('inf'), float('-inf')], float('nan'))
                            converted = converted.fillna(0.0)
            ...
            try:
                ...
                try:
                    from pandas import to_numeric
                    converted = to_numeric(result, errors='coerce')
                    converted = converted.replace([float('inf'), float('-inf')], float('nan'))
                    converted = converted.fillna(0.0)
                    df.loc[:, (ind_name)] = converted
                except:
                    pass  # Skip this indicator
